...
namespace WebMVC.Applications.DomainEventHandlers.PairingDomain
{
    ...
    /// </summary>
    public class QrCodeQuotaUpdatedDomainEventHandler
            : INotificationHandler<QrCodeQuotaUpdatedDomainEvent>
    {
        private readonly IQrCodeQueries _qrCodeQueries;

        public QrCodeQuotaUpdatedDomainEventHandler(IQrCodeQueries qrCodeQueries)
        {
            _qrCodeQueries = qrCodeQueries ?? throw new ArgumentNullException(nameof(qrCodeQueries));
        }

        public async Task Handle(QrCodeQuotaUpdatedDomainEvent domainEvent, CancellationToken cancellationToken)
        {
            ...
            if (qrCodeVM != null)
            {
                qrCodeVM.DailyAmountLimit = (int)qrCode.DailyAmountLimit;
                qrCodeVM.OrderAmountUpperLimit = (int)qrCode.OrderAmountUpperLimit;
                qrCodeVM.OrderAmountLowerLimit = (int)qrCode.OrderAmountLowerLimit;
