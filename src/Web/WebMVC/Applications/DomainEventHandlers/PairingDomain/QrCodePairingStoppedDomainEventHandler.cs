...
namespace WebMVC.Applications.DomainEventHandlers.PairingDomain
{
    ...
    /// </summary>
    public class QrCodePairingStoppedDomainEventHandler
            : INotificationHandler<QrCodePairingStoppedDomainEvent>
    {
        private readonly IQrCodeQueries _qrCodeQueries;

        public QrCodePairingStoppedDomainEventHandler(IQrCodeQueries qrCodeQueries)
        {
            _qrCodeQueries = qrCodeQueries ?? throw new ArgumentNullException(nameof(qrCodeQueries));
        }

        public async Task Handle(QrCodePairingStoppedDomainEvent domainEvent, CancellationToken cancellationToken)
        {
            ...
            if (qrCodeVM != null)
            {
                qrCodeVM.PairingStatus = qrCode.GetPairingStatus.Name;
                qrCodeVM.PairingStatusDescription = qrCode.PairingStatusDescription;
