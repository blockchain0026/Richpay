...
namespace WebMVC.Applications.DomainEventHandlers.PairingDomain
{
    ...
    /// </summary>
    public class QrCodePairingStartedDomainEventHandler
            : INotificationHandler<QrCodePairingStartedDomainEvent>
    {
        private readonly IQrCodeQueries _qrCodeQueries;

        public QrCodePairingStartedDomainEventHandler(IQrCodeQueries qrCodeQueries)
        {
            _qrCodeQueries = qrCodeQueries ?? throw new ArgumentNullException(nameof(qrCodeQueries));
        }

        public async Task Handle(QrCodePairingStartedDomainEvent domainEvent, CancellationToken cancellationToken)
        {
