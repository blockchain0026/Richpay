...
namespace WebMVC.Applications.DomainEventHandlers.PairingDomain
{
    ...
    /// </summary>
    public class QrCodeDisabledDomainEventHandler
            : INotificationHandler<QrCodeDisabledDomainEvent>
    {
        private readonly IQrCodeQueries _qrCodeQueries;

        public QrCodeDisabledDomainEventHandler(IQrCodeQueries qrCodeQueries)
        {
            _qrCodeQueries = qrCodeQueries ?? throw new ArgumentNullException(nameof(qrCodeQueries));
        }

        public async Task Handle(QrCodeDisabledDomainEvent domainEvent, CancellationToken cancellationToken)
        {
            ...
            if (qrCodeVM != null)
            {
                ...
                qrCodeVM.CodeStatusDescription = qrCode.CodeStatusDiscription;
