...
var KTUserListDatatable = function () {
	...
	const aspController = 'TraderAgent';
	...
	var init = function () {
		...
		datatable = $('#kt_apps_traderagent_list_datatable').KTDatatable({
			...
			columns: [{
				...
				overflow: 'visible',
				template: function (row) {
					var updateLink = '/' + aspController + '/' + aspActionUpdate + '?traderAgentId=' + row.TraderAgentId;
					return '\
							<div class="dropdown">\
								...
								<div id="row_action_dropdown_menu" class="dropdown-menu dropdown-menu-right">\
									<ul class="kt-nav">\
										...
										<li class="kt-nav__item">\
											<a href="#" data-user-id="' + row.TraderAgentId + '" class="kt-nav__link" data-toggle="show-modal_bankbook_records_list_datatable" data-target="#modal_bankbook_records_list_datatable">\
												<i class="kt-nav__link-icon fa fa-search-dollar"></i>\
												<span class="kt-nav__link-text">帐变纪录</span>\
											</a>\
										</li>\
										<li class="kt-nav__item">\
											<a href="#" data-user-id="' + row.TraderAgentId + '" class="kt-nav__link" data-toggle="show-modal_running_account_records_datatable" data-target="#modal_running_account_records_datatable">\
												<i class="kt-nav__link-icon fa fa-money-bill-wave""></i>\
												<span class="kt-nav__link-text">流水纪录</span>\
											</a>\
										</li>\
										<li class="kt-nav__item">\
											<a href="#modal__change_balance" data-user-id="' + row.TraderAgentId + '" data-current-balance="' + row.Balance.AmountAvailable.toFixed(3) + '" data-toggle="show-modal-change-balance" class="kt-nav__link" >\
										...
										<li class="kt-nav__item">\
											<a href="#modal__frozen_balance" data-user-id="' + row.TraderAgentId + '" data-current-balance="' + row.Balance.AmountAvailable.toFixed(3) + '" data-toggle="show-modal_frozen_balance_list_datatable" class="kt-nav__link" >\
	...
	var modalUnfreeze = function (id, amount) {
		...
		swal.fire({
			...
			text: "確定要取消此筆凍結嗎?  将归还 " + amount + "¥ 给代理。",
			type: "danger",
		...
		$('#kt_subheader_group_actions_status_change').on('click', "[data-toggle='status-change']", function () {
			...
			if (ids.length > 0) {
				...
				swal.fire({
					...
					html: "确定要修改选取的 " + ids.length + " 個代理帳戶狀態為 " + status + " 吗?",
					type: "info",
					...
					preConfirm: () => {
						return new Promise(function (resolve, reject) {
							$.ajax({
								url: "TraderAgent/UpdateAccountStatus",
								type: "POST",
								contentType: "application/json",
		...
		$('#kt_subheader_group_actions_delete_all').on('click', function () {
			...
			if (ids.length > 0) {
				...
				swal.fire({
					...
					text: "确定要删除选取的 " + ids.length + " 个代理帐户吗 ?",
					type: "danger",
