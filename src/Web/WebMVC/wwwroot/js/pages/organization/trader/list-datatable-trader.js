...
var KTUserListDatatable = function () {
	...
	var init = function () {
		...
		datatable = $('#kt_apps_trader_list_datatable').KTDatatable({
			...
			columns: [{
					...
					if (number > 5) {
						...
						output = '<div class="kt-user-card-v2">\
								...
								<div class="kt-user-card-v2__details">\
									<a href="#kt_modal_quick_view_user_info" data-record-id="' + data.TraderId + '" class="kt-user-card-v2__name">' + data.FullName + '</a>\
					...
					if (row.UplineUserId) {
						output = '<div class="kt-user-card-v2">\
								<div class="kt-user-card-v2__details kt-container">\
									<a href="#kt_modal_quick_view_user_info " data-record-id="' + uplineUserId + '" data-is-trader-agent=true class="kt-user-card-v2__name">' + uplineFullName + '</a>\
								</div>\
							</div>';
					}
					else {
				...
				overflow: 'visible',
				template: function (row) {
					var updateLink = '/' + aspController + '/' + aspActionUpdate + '?traderId=' + row.TraderId;
					return '\
							<div class="dropdown">\
								...
								<div id="row_action_dropdown_menu" class="dropdown-menu dropdown-menu-right">\
									<ul class="kt-nav">\
										...
										<li class="kt-nav__item">\
											<a href="#" data-user-id="' + row.TraderId + '" class="kt-nav__link" data-toggle="show-modal_bankbook_records_list_datatable" data-target="#modal_bankbook_records_list_datatable">\
												<i class="kt-nav__link-icon fa fa-search-dollar"></i>\
												<span class="kt-nav__link-text">帐变纪录</span>\
											</a>\
										</li>\
										<li class="kt-nav__item">\
											<a href="#" data-user-id="' + row.TraderId + '" class="kt-nav__link" data-toggle="show-modal_running_account_records_datatable" data-target="#modal_running_account_records_datatable">\
												<i class="kt-nav__link-icon fa fa-money-bill-wave"></i>\
												<span class="kt-nav__link-text">流水纪录</span>\
											</a>\
										</li>\
										<li class="kt-nav__item">\
											<a href="#modal__change_balance" data-user-id="' + row.TraderId + '" data-current-balance="' + row.Balance.AmountAvailable.toFixed(3) + '" data-toggle="show-modal-change-balance" class="kt-nav__link" >\
										...
										<li class="kt-nav__item">\
											<a href="#modal__frozen_balance" data-user-id="' + row.TraderId + '" data-current-balance="' + row.Balance.AmountAvailable.toFixed(3) + '" data-toggle="show-modal_frozen_balance_list_datatable" class="kt-nav__link" >\
		...
		datatable.on('click', '[data-record-id]', function () {
			modalUserDetail($(this).data('record-id'), $(this).data('is-trader-agent'));
		...
		downline_datatable = $('#kt_apps_downline_list_datatable').KTDatatable({
			...
			data: {
				...
				source: {
					read: {
						...
						url: 'TraderAgent/SearchDownlines',
					},
			...
			columns: [{
				...
				template: function (data, i) {
					...
					if (number > 5) {
						...
						output = '<div class="kt-user-card-v2">\
								...
								<div class="kt-user-card-v2__details">\
									<a href="#'+ data.TraderAgentId + '" data-record-id="' + data.TraderAgentId + '" class="kt-user-card-v2__name">' + data.FullName + '</a>\
									<span class="kt-user-card-v2__desc">' + data.Username + '</span>\
								</div>\
							</div>';
					} else {
				...
				overflow: 'visible',
				template: function (row) {
					return '<button data-trader-agent-id="' + row.TraderAgentId + '" data-toggle="show-modal-downline_trader" class="btn btn-sm btn-font-sm btn-elevate btn-elevate-air btn-brand">' + '查看交易员' + '</button>';
				},
			}]
		});
		...
		downline_datatable.on('click', '[data-toggle="show-modal-downline_trader"]', function () {
			var modal = $('#modal_downline_trader_list_datatable');
			var id = $(this).data('trader-agent-id');
			modalDownlineTraderDatatable(id);
			modal.modal('show');
		});
		...
		downline_trader_datatable = $('#datatable_downline_trader').KTDatatable({
			// datasource definition
			data: {
				type: 'remote',
				source: {
					read: {
						url: 'Trader/SearchDownlines'
					}
				},
				pageSize: 10, // display 10 records per page
				serverPaging: true,
		...
		downline_trader_datatable.hide();
	...
	// user details
	var modalUserDetail = function (id, isTraderAgent) {
		var el = $('#modal_sub_datatable_ajax_source');
		var requestUrl = "";
		var requestData;

		if (isTraderAgent) {
			requestUrl = "TraderAgent/Detail";
			requestData = { traderAgentId: id };
		}
		else {
			requestUrl = "Trader/Detail";
			requestData = { traderId: id };
		}
		$.ajax({
			url: requestUrl,
			type: "Get",
						...
						swal.fire({
							...
							if (result.value) {
								...
								if (currentTableIndex == 1) {
									...
									downline_trader_datatable.reload();
								}
	...
	// downline traders
	var modalDownlineTraderDatatable = function (id) {

		//Selection
		downline_trader_datatable.on('kt-datatable--on-check kt-datatable--on-uncheck kt-datatable--on-layout-updated', function (e) {
			var checkedNodes;
			checkedNodes = downline_trader_datatable.rows('.kt-datatable__row--active').nodes(); // get selected records
			var count = checkedNodes.length; // selected records count

			$('#downline_trader_datatable_group_selected_rows').html(count);

			if (count > 0) {
				$('#downline_trader_datatable_search').addClass('kt-hidden');
				$('#downline_trader_datatable_group_actions').removeClass('kt-hidden');
			} else {
				$('#downline_trader_datatable_search').removeClass('kt-hidden');
				$('#downline_trader_datatable_group_actions').addClass('kt-hidden');
			}
		});

		//Change status
		$('#downline_trader_datatable_group_actions_status_change').on('click', "[data-toggle='status-change']", function () {
			...
			// fetch selected IDs
			var ids;
			ids = downline_trader_datatable.rows('.kt-datatable__row--active').nodes().find('.kt-checkbox--single > [type="checkbox"]').map(function (i, chk) {
				return $(chk).val();
			});
		...
		//Update total
		downline_trader_datatable.on('kt-datatable--on-layout-updated', function () {
			$('#downline_trader_datatable_total').html('共 ' + downline_trader_datatable.getTotalRows() + ' 笔');
		});

		//Show user details
		downline_trader_datatable.on('click', '[data-record-id]', function () {
			modalUserDetail($(this).data('record-id'), $(this).data('is-trader-agent'));
			$('#kt_modal_quick_view_user_info').modal('show');
		});
		...
		modal.on('shown.bs.modal', function () {
			...
			var modalContent = $(this).find('.modal-content');
			downline_trader_datatable.spinnerCallback(true, modalContent);
			...
			downline_trader_datatable.setDataSourceParam('query', queryObejct);
			downline_trader_datatable.load();
			downline_trader_datatable.search();

			downline_trader_datatable.on('kt-datatable--on-layout-updated', function () {
				downline_trader_datatable.show();
				downline_trader_datatable.spinnerCallback(false, modalContent);
				downline_trader_datatable.redraw();
			});
	...
	var modalUnfreeze = function (id, amount) {
		...
		swal.fire({
			...
			text: "確定要取消此筆凍結嗎?  将归还 " + amount + "¥ 给交易员。",
			type: "danger",
			...
			if (ids.length > 0) {
				...
				swal.fire({
					...
					preConfirm: () => {
						return new Promise(function (resolve, reject) {
							$.ajax({
								url: "Trader/UpdateAccountStatus",
								type: "POST",
								contentType: "application/json",
								data: JSON.stringify(accounts),
								success: function (response) {
		...
		$('#kt_subheader_group_actions_delete_all').on('click', function () {
			...
			if (ids.length > 0) {
				...
				swal.fire({
					...
					text: "确定要删除选取的 " + ids.length + " 个交易员帐户吗 ?",
					type: "danger",
