...
var KTUserListDatatable = function () {
	...
	var init = function () {
		...
		datatable.columns('ShopAgentId').visible(selectorVisibility);
	...
	var selectedDelete = function () {
		$('#kt_subheader_group_actions_delete_all').on('click', function () {
			...
			if (ids.length > 0) {
				...
				//build post data.
				idList.forEach(function (shopAgentId) {
					let account = {
						UserId: shopAgentId, IsReviewed: false
					};
					accounts.push(account);
				});
	...
	var selectedReview = function () {
		$('#kt_subheader_group_actions_review_all').on('click', function () {
			...
			if (ids.length > 0) {
				...
				//build post data.
				idList.forEach(function (shopAgentId) {
					console.log(shopAgentId);
					let account = {
						UserId: shopAgentId, IsReviewed: true
					};
					accounts.push(account);
				});
				...
				swal.fire({
					...
					text: "确定要让选取的 " + ids.length + " 个商户代理帐户通过审核吗 ?",
					type: "danger",
